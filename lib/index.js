...
function parseOther(value) {
  ...
  try {
    ...
    return { ...cacheTokens !== void 0 ? { cacheTokens } : {} };
  } catch {
    return {};
  }
}
async function fetchRecentCalls(origin, accessKey, units) {
  try {
    const { status, body } = await getJson(origin, "/api/log/self", accessKey, {
      type: 2,
      page: 1,
      page_size: LOG_PAGE_SIZE,
      start_timestamp: Math.floor(localDayStartMs() / 1e3),
      end_timestamp: Math.floor(Date.now() / 1e3)
    });
...
async function readNewApi(account, accessKey) {
  ...
  let todayTokens;
  {
    ...
    let outputTokens;
    let cacheReadTokens;
    if (recentCalls !== void 0) {
      for (const call of recentCalls) {
        if (call.promptTokens !== void 0) inputTokens = (inputTokens ?? 0) + call.promptTokens;
        if (call.completionTokens !== void 0) outputTokens = (outputTokens ?? 0) + call.completionTokens;
        if (call.cacheTokens !== void 0) cacheReadTokens = (cacheReadTokens ?? 0) + call.cacheTokens;
      }
    }
